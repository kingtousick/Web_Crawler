...
def crawl_news():
    ...
    try:
        # 뉴스 검색 URL 생성
        url = f"https://search.naver.com/search.naver?where=news&query={keyword}&sm=tab_opt&sort=0&photo=0&field=0&reporter_article=&pd=3&ds={start_date}&de={end_date}&docid=&nso=so:r,p:from{start_date.replace('.', '')}to{end_date.replace('.', '')},a:all&mynews=0&cluster_rank=37&start=1&refresh_start=0"
        print(url)
        # HTTP GET 요청
        ...
        for news in news_list:
            ...
            # 날짜 추출
            date_element = news.select_one('.info_group > span:not(.spnew.ico_paper)')
